...
The Store target and website target are maintained from shared code. Store **1.0 (5)** was submitted; see RELEASE-5.md and fresh App Store Connect state. Website **1.5.3** restores direct distribution to avoid per-card sandbox authorization for folder detection and cleaning. This supersedes earlier Store-only notes.
...
- Direct identity: `com.ryansmithphotography.Ejector`, version 1.5.3. At Ryan's earlier request, `/Applications/Easy Eject.app` now contains the notarized local Store build. The old executable is retained in release evidence for rollback.
